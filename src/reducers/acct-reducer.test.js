...
describe('acct-reducer', () => {
	...
	it(`should handle ${types.GET_ACCTS_SUCCESS}`, () => {
		expect(reducer(undefined, accData.getAccts.action)).toEqual(accData.getAccts.state)
	})
	it(`should handle ${types.GET_ASSOC_PROJS_SUCCESS}`, () => {
		expect(reducer(undefined, accData.getAssocProjs.action)).toEqual(accData.getAssocProjs.state)
	})
})
