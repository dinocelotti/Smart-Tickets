...
export const EntityTypes = {
	queryBuyer: (_buyer, _typeOfTix) =>
		makeMethod('queryBuyer', _buyer, encodeString(_typeOfTix)),
...
export const PromoTypes = {
	...
	addIpfsDetailsToTix: (_typeOfTix, _hash) =>
		makeMethod('addIpfsDetailsToTix', encodeString(_typeOfTix), _hash),

	setTixPrice: (_typeOfTix, _priceInWei) =>
		makeMethod('setTixPrice', encodeString(_typeOfTix), _priceInWei),

	setTixQuantity: (_typeOfTix, _quantity) =>
		makeMethod('setTixQuantity', encodeString(_typeOfTix), _quantity),

	addDistrib: _distrib => makeMethod('addDistrib', _distrib),

	setDistribAllotQuan: (_distrib, _typeOfTix, _quantity) =>
		makeMethod(
			'setDistribAllotQuan',
			_distrib,
			encodeString(_typeOfTix),
			_quantity
		),

	setDistribFee: (_distrib, _promosFee) =>
		makeMethod('setDistribFee', _distrib, _promosFee)
}
export const BuyerTypes = {
	setMarkup: (_markup, _typeOfTix) =>
		makeMethod('setMarkup', _markup, encodeString(_typeOfTix)),

	buyTixFromPromo: (_typeOfTix, _quantity) =>
		makeMethod('buyTixFromPromo', encodeString(_typeOfTix), _quantity),
